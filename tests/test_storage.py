...
from gcontact_sync.storage.db import SCHEMA, SyncDatabase
...
class TestSyncDatabaseInitialization:
    ...
    def test_initialize_creates_tables(self):
        ...
        with db.connection() as conn:
            ...
            cursor = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='contact_mapping'"
            )
            assert cursor.fetchone() is not None
    ...
    def test_initialize_creates_indexes(self):
        ...
        with db.connection() as conn:
            cursor = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_contact_mapping_key'"
            )
            assert cursor.fetchone() is not None

            cursor = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_sync_state_account'"
            )
            assert cursor.fetchone() is not None
...
class TestConnectionContextManager:
    ...
    def test_connection_rollback_on_error(self):
        ...
        with db.connection() as conn:
            ...
            row = cursor.fetchone()
            # Note: SQLite autocommit behavior may vary, but error should have prevented insert
...
class TestContactMappingOperations:
    ...
    def test_get_all_contact_mappings_returns_all(self, db):
        ...
        db.upsert_contact_mapping(
            matching_key='alice:alice@example.com',
            account1_resource_name='people/1'
        )
        db.upsert_contact_mapping(
            matching_key='bob:bob@example.com',
            account1_resource_name='people/2'
        )
        db.upsert_contact_mapping(
...
class TestTimestampHandling:
    ...
    def test_contact_mapping_has_timestamps(self, db):
        ...
        # Timestamps may be returned as strings or datetime objects depending on SQLite config
...
class TestEdgeCases:
    ...
    def test_concurrent_updates_same_mapping(self, db):
        ...
        for i in range(100):
            db.upsert_contact_mapping(
                matching_key='concurrent_test',
                account1_resource_name=f'people/{i}'
            )
    ...
    def test_file_database_persists(self, tmp_path):
        ...
        db1.initialize()
        db1.upsert_contact_mapping(matching_key='persistent', account1_resource_name='p1')
